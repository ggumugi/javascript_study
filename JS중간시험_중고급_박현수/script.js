...
let aver = avg(scores).toFixed(0)
console.log(`3번 문제 정답: ${aver}점`)
...
/*
    ...
    아래 콘솔에 출력하세요.
*/
phonenum = student.phone.replaceAll('-', '')
console.log(`8번 문제 정답: ${phonenum}`)
...
*/
let date = new Date(student.reg_date)
...
employees.forEach((em) => {
   if (em.addr.indexOf('서울') > -1) {
      console.log('서울에 삽니다.')
   } else {
      console.log('서울에 살지 않습니다.')
   }
})
let newaddr = employees.filter((em) => {
   return em.addr.indexOf('서울') > -1
})
console.log(`10번 문제 정답: ${JSON.stringify(newaddr, null, 2)}`)
...
*/
let newmap = employees.map((em) => {
   em.dept = '개발부'
   return em
})

console.log(`11번 문제 정답: ${JSON.stringify(newmap, null, 2)}`)
